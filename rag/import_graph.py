...
class Neo4jGraphImporter:
        ...
        with self.driver.session() as session:
            for entity in entities:
                ...
                props = {
                    "name": entity["name"],
                    "type": entity["type"],
                    "description": entity["description"] if entity.get("description") else "",
    ...
    def import_claims(self) -> int:
        ...
        with self.driver.session() as session:
            for claim in claims:
                ...
                # Create the claim and connect to subject entity
                query = """
                ...
                # Add connection to object entity if it exists
                if claim.get("object"):
                    query += """
                    WITH c
    ...
    def _sanitize_relationship_type(self, rel_type: str) -> str:
        """Sanitize relationship type to conform to Neo4j requirements"""
    ...
    def import_graph(self) -> Dict[str, int]:
        """Import all entities, relationships, and claims into Neo4j"""
        try:
            # Connect to Neo4j
            self.connect()
...
def main():
    """Main function for importing the knowledge graph into Neo4j"""
