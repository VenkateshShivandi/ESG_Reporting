...
def store_document_record(filename: str, source_type: str, status: str = "processed") -> Optional[str]:
    """
    ...
    # Use naive timestamp if DB expects timestamp without timezone
    # upload_timestamp = datetime.now().isoformat() # Naive
    upload_timestamp_utc = datetime.now(timezone.utc).isoformat() # With TZ

    document_data = {
        "id": document_uuid,               # Map to DB column 'id'
        "file_name": filename,
        "uploaded_at": upload_timestamp_utc, # Map to DB column 'uploaded_at' (Supabase handles TZ conversion)
        # "status": status,                # Column does not exist
        "file_type": source_type,          # Map to DB column 'file_type'
        # DB columns user_id, size, file_path are not populated here
    }

    try:
        logger.info(f"Storing document record for: {filename} (ID: {document_uuid}) in schema 'esg_data'")
        # Specify schema using .schema() method before .table()
        response = supabase.schema("esg_data").table(DOCUMENTS_TABLE).insert(document_data).execute()

        # Check for errors in the response
        if hasattr(response, 'error') and response.error:
            logger.error(f"Error storing document record for {filename}: {response.error}")
            return None
        # Check if data was actually returned (indicates success)
        if not response.data:
             logger.warning(f"Supabase insert for document {filename} returned no data, though no explicit error.")
             # Continue assuming success if no error, but log warning.
             
        logger.info(f"Successfully stored document record for {filename}. Document ID: {document_uuid}")
...
def store_chunks(document_id: str, chunks: List[Dict[str, Any]], embeddings: List[List[float]]) -> bool:
    """
    ...
    if not chunks:
        logger.info(f"No chunks provided for document ID {document_id}. Nothing to store.")
        return True
    ...
    try:
        logger.info(f"Storing {len(records_to_insert)} chunks for document ID: {document_id} in schema 'esg_data'")
        # Specify schema using .schema() method before .table()
        response = supabase.schema("esg_data").table(CHUNKS_TABLE).insert(records_to_insert).execute()

        # Basic check for errors
        if hasattr(response, 'error') and response.error:
            logger.error(f"Error storing chunks for document {document_id}: {response.error}")
            return False
        if not response.data:
             logger.warning(f"Supabase insert for chunks of document {document_id} returned no data, though no explicit error.")
             # Continue assuming success if no error.

        logger.info(f"Successfully stored {len(records_to_insert)} chunks for document ID: {document_id}")
        return True
