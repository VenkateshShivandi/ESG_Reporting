...
import { Folder, FileText, FileSpreadsheet, FileType as LucideFileTypeIcon, FileCheck, File as DefaultFileIcon } from 'lucide-react';
...
documentsApi.listFiles = vi.fn(async (path: string[] = []) => {
  ...
  if (path.length === 0) return mockFilesStore.filter(f => f.path.length === 0 && f.name !== '.folder');
  if (JSON.stringify(path) === JSON.stringify(['FolderA'])) {
    return mockFilesStore.filter(f => JSON.stringify(f.path) === JSON.stringify(['FolderA']) && f.name !== '.folder');
  }
  return [];
});

documentsApi.getDownloadUrl = vi.fn(async (filePath: string) => {
  if (filePath === 'Test.pdf' || filePath.endsWith('Test.pdf')) { // Make it more robust
...
describe("Navigation Functionality", () => {
  beforeEach(() => {
    ...
    documentsApi.listFiles = vi.fn(async (path: string[] = []) => {
      if (path.length === 0) return mockFilesStore.filter(f => f.path.length === 0 && f.name !== '.folder');
      if (JSON.stringify(path) === JSON.stringify(['FolderA'])) return mockFilesStore.filter(f => JSON.stringify(f.path) === JSON.stringify(['FolderA']) && f.name !== '.folder');
  ...
  it("should navigate into a folder when its name is clicked", async () => {
    ...
    if (!folderAElement) throw new Error("FolderA clickable element (parentElement of text) not found");
  ...
  it("should navigate to a parent folder when its breadcrumb is clicked", async () => {
    ...
    const folderABreadcrumbActive = screen.getByText((content, element) => 
...
describe("UI State and Loading Indicators", () => {
  ...
  it("should show 'No files in this folder' message for an empty folder", async () => {
    ...
    documentsApi.listFiles = vi.fn().mockImplementation(async (path: string[] = []) => {
      if (path.length === 0) { // If navigating back to root for some reason
  ...
  it("should correctly manage opening and closing of all dialogs", async () => {
    ...
    const closeCreateFolderDialogXButton = within(createFolderDialog).getByRole('button', { name: "Close" });
    ...
    try {
      ...
      }, { timeout: 1000 }); // Shorter timeout for this check
    } catch (e) {
    ...
    const renameMenuItem = await screen.findByRole('menuitem', { name: /Rename/i }, { timeout: 4000 });
    ...
    const previewMenuItem = await screen.findByRole('menuitem', { name: /Preview/i }, { timeout: 4000 });
...
describe("Drag and Drop - Create Folder from Files", () => {
  ...
  it("should open 'Create New Folder' dialog when a file is dropped onto another file in the same folder", async () => {
    ...
    if (!sourceNode || !targetNode) {
      throw new Error('Could not find draggable/droppable nodes for DND test. Check selectors or DraggableFileItem structure.');
    }
    ...
    expect(screen.getByText(/Enter a name for the new folder that will contain the selected files./i)).toBeInTheDocument();
  });
  ...
  it("should create a new folder and move the two files into it", async () => {
    ...
    const createFolderDialogTitle = await screen.findByRole('heading', { name: /Create New Folder/i, level: 2 });
    ...
    const createButton = within(dialogElement as HTMLElement).getByRole('button', { name: /Create Folder/i });
    ...
    // Dialog should close
    await waitFor(() => {
      expect(screen.queryByRole('heading', { name: /Create New Folder/i, level: 2 })).not.toBeInTheDocument();
    });
